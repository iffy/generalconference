...
from download import extractTalkAsMarkdown, getURL


class extractTalkAsMarkdownTest(TestCase):

    def assertEqualDiff(self, expected, actual, message=None):
        alines = [repr(x)+'\n' for x in expected.split('\n')]
        blines = [repr(x)+'\n' for x in actual.split('\n')]
        diff = ''.join(difflib.unified_diff(alines, blines,
            fromfile='expected', tofile='actual'))
        if expected != actual:
            self.fail("{message}\n\n{diff}".format(**locals()))

    def test_it_works_wonderfully(self):
        url = 'https://www.lds.org/general-conference/2015/10/it-works-wonderfully?lang=eng'
        html = getURL(url)
        metadata = {}
        actual = extractTalkAsMarkdown(html, metadata)
        open(self.mktemp(), 'wb').write(actual)
...
Therefore, as leaders we must strictly protect the Church and the gospel in
...
I learned in my life that we don't need to be "more" of anything to start to
...
This is my hope, my prayer, my testimony, and my blessing in the name of Jesus
Christ, amen.
